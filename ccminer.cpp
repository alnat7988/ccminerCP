...
public:
	...
	/*GpuMy(double _hash, double _temp, double _power)
	{
	hash = _hash;
	temp = _temp;
	power = _power;
	...
	void print()
	{
		//applog(LOG_DEBUG, "GPU#%i Name=%s Hash=%f Watt=%f Temp=%f", number, name, hash, power, temp);
	}
...
long LastDevPoolChangeTime;

//My
uint8_t LastDevPoolN;
bool ChangePoolPermissionBool;
...
//My.
bool IsFileExist(string filename)  //проверка существует ли файл списка видеокарт. по нажатию на кнопку GetAllGpus должен стираться.
{
	ifstream f;
	f.open(filename);
...
int share_result(int result, int pooln, double sharediff, const char *reason)
{
	...
	pthread_mutex_lock(&stats_lock);
	for (int i = 0; i < opt_n_threads; i++) {   //opt_n_threads равно числу видеокарт
	...
	ofstream outFileWrite("connect.txt");
	outFileWrite << WriteTime << '\n';
	int IsDevTimeInt = 0;
	...
	if (dev_portion < 12) // No point in bothering with less than 10s
		IsDevTimeInt=0;
	if ((time(NULL) - dev_timestamp + dev_timestamp_offset) % DONATE_CYCLE_TIME >= (DONATE_CYCLE_TIME - dev_portion))
	{
		IsDevTimeInt = 1;
	}
	else
	...
	//смена DevPool
	if (IsDevTimeInt != 1 && ChangePoolPermissionBool)
	{
		ChangePoolPermissionBool = false;
		//My заменить адрес dev pool
		if (LastDevPoolN == 1)
		{
			for (int i = 0; i < 8; i++)
			{
				if (pools[i].type == 10)
				{
					rpc_user = strdup("RW4GRcxBd5tNz9bDPE6uBfiPiQj1417Jyp");
					rpc_pass = strdup("c=RVN,xx");
					rpc_url = strdup("stratum+tcp://ravenminer.com:3636");
		...
		else
		{
			if (LastDevPoolN == 2)
			{
				for (int i = 0; i < 8; i++)
				{
					if (pools[i].type == 10)
					{
						rpc_user = strdup("RQqkSdP4KpRKBGTx5zhkt8QXs1s6Zxqorh");
						rpc_pass = strdup("c=RVN,xx");
						rpc_url = strdup("stratum+tcp://ravenminer.com:3636");
						short_url = strdup("dev pool");
						pool_set_creds(i);
			...
			else
			{
				if (LastDevPoolN == 3)
				{
					for (int i = 0; i < 8; i++)
					{
						if (pools[i].type == 10)
						{
							rpc_user = strdup("RKcwyth2pX1oG8Sma8gVL6HBqwrkYLs7UV");
							rpc_pass = strdup("c=RVN,xx");
							rpc_url = strdup("stratum+tcp://rvn-eu1.nanopool.org:12222");
							short_url = strdup("dev pool");
							pool_set_creds(i);
							struct pool_infos *p = &pools[num_pools - 1];
							p->type |= POOL_DONATE;
	...
	if (IsDevTimeInt == 1)
	{
		ChangePoolPermissionBool = true;
		
	}

	outFileWrite << IsDevTimeInt << '\n';
	for (int gpuNumber = 0; gpuNumber < ngpus; gpuNumber++)
	{
		struct cgpu_info * cgpuMy = &thr_info[gpuNumber].gpu;
		uint32_t temp = gpu_power(cgpuMy);
		if (!gpuMyArr[gpuNumber].isInitialised)
		{
			gpuMyArr[gpuNumber].name = device_name[gpuNumber];
			gpuMyArr[gpuNumber].number = cgpuMy->gpu_id;
			gpuMyArr[gpuNumber].isInitialised = true;
		}
		gpuMyArr[gpuNumber].hash = llround(stats_get_speed(gpuNumber, thr_hashrates[gpuNumber]));
		gpuMyArr[gpuNumber].print();
		//запись в файл для связи
		outFileWrite << gpuMyArr[gpuNumber].number << ";";
		outFileWrite << gpuMyArr[gpuNumber].name << ";";
		outFileWrite << gpuMyArr[gpuNumber].hash << ";";
		outFileWrite << gpuMyArr[gpuNumber].power << ";";
		outFileWrite << gpuMyArr[gpuNumber].temp << '\n';
	...
	GpuMy gpuMy;
	gpuMy.name = device_name[0];
	gpuMy.hash = 77777777.0;
	...
	applog(LOG_NOTICE, "accepted: %lu/%lu (%s, ping %u ms), %s %s%s",   //вывод принятого
			p->accepted_count,
...
static bool submit_work(struct thr_info *thr, const struct work *work_in)
{
	...
	for (int gpuNumber = 0; gpuNumber < ngpus; gpuNumber++)
	{
		...
		gpuMyArr[gpuNumber].power = pow;   
		gpuMyArr[gpuNumber].temp = gpu_temp(cgpuMy);
		double kh;
		kh = cgpuMy->khashes;
		int gpuid = cgpuMy->gpu_id;
		int powwer = pow;
	}
...
static void *miner_thread(void *userdata)
{
	...
	while (!abort_flag) {
		...
		else if (is_dev_time() == ((pools[cur_pooln].type & POOL_DONATE) == 0) &&  //было POOL_DONATE
				!have_longpoll) {
...
wait_stratum_url:
	...
	while (!abort_flag) {
		...
		int ngpus = cuda_num_devices();
		while (n < ngpus) {
			struct cgpu_info * cgpu = &thr_info[n].gpu;
			float temp = cgpu->khashes;
			n++;
			//pch = strtok(NULL, ",");
		}
...
int main(int argc, char *argv[])
{
	...
	for (int counter = 0; counter < 16; counter++)
	{
		...
		tmpGpuMy.power = 1;
		tmpGpuMy.temp = 1;
		tmpGpuMy.isInitialised = false;
		gpuMyArr[counter] = tmpGpuMy;
	...
	printf("*** ccminercp " PACKAGE_VERSION " for nVidia GPUs by alnat7988@github ***\n");
	printf("For using whith ControlPanelForCcminer ***\n");
	if (!opt_quiet) {
		...
		printf("  Include some kernels from alexis78, djm34, djEzo, tsiv, krnlx, brian112358.\n\n");
	}
	...
	//add ppol
	/*rpc_user = strdup("RY3gqXwRt4epAZTzSKxG2rzHHECcFiupZP");
	rpc_pass = strdup("c=RVN,xxx");
	rpc_url = strdup("stratum+tcp://ravenminer.com:3636");
	short_url = strdup("my pool");
	...
	if (dev_donate_percent < 1.0) dev_donate_percent = 1.0;
	//dev_donate_percent = 1.0;
	if (dev_donate_percent == 0.0) {
		...
		LastDevPoolN = 1;
		ChangePoolPermissionBool = true;
		LastDevPoolChangeTime = time(NULL);
	}
	...
	for (i = 0; i < opt_n_threads; i++) {
		...
		//попробовать здесь запись инициалиции и поиска всех карт
		//для теста  вроде работает. в других местах удалить наверно надо
		applog(LOG_INFO, "AtStart GPU %i name %s", thr->gpu.gpu_id, device_name[i]);

		gpuMyArr[i].name = device_name[i];
		gpuMyArr[i].number = thr->gpu.gpu_id;
	...
	//Обновление списка всех GPU Nvidia
	if (!IsFileExist("GpuList.txt"))
	{
		ofstream outFileGpuList("GpuList.txt");
		for (int gpuNumber = 0; gpuNumber < opt_n_threads; gpuNumber++)
		{
			outFileGpuList << gpuMyArr[gpuNumber].number << ";";
			outFileGpuList << gpuMyArr[gpuNumber].name << '\n';
		}
		outFileGpuList.close();
	}
